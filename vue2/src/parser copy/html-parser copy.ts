import { isNonPhrasingTag, isPlainTextElement, makeMap, no } from "../utils"
...
export function parseHTML(html: string, options: any) {
  ...
  while (html) {
    last = html
    // <div>{{message}}</div>
    // script、style、textarea 是父标签，它的内容都是文本
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        // 获取到开始1.标签名，以及标签上的2.属性3.结尾，<div class="box"> 解析完成
